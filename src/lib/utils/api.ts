...
export const authPocketbase = async (user: string, password: string) => {
	const res = await pb.collection('users_altlokalt').authWithPassword(user, password);
...
export const createPocketbaseUser = async (data: any) => {
	const res = await pb.collection('users_altlokalt').create(data);
	authData.set(res);

	// (optional) send an email verification request
	await pb.collection('users_altlokalt').requestVerification(data.email);
...
export const refreshAuthPocketbase = async () => {
	...
	const user = await pb.collection('users_altlokalt').authRefresh({
